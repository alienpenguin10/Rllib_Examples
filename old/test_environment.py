...
class TemporalDictEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Check for NaN in observation (should trigger truncation)
        obs_has_nan = self._check_for_nan(self.current_obs)
        if obs_has_nan:
            print("WARNING: NaN detected in observation - truncating episode")
            reward = 0.0
            terminated = False
            truncated = True
            info = {
                'step_count': self.step_count,
                'reason': 'nan_observation',
                'temporal_history_length': len(self.temporal_history)
            }
    ...
    def _update_observation(self, action):
        ...
        if len(self.temporal_history) > 1:
            # Use history of actions to influence observation
            recent_actions = self.temporal_history[-2:]
            temporal_factor = np.mean([
                np.mean(h['action']['continuous_action']) for h in recent_actions
            ]) * 0.05
    ...
    def _calculate_reward(self, obs):
        ...
        # Reward for hitting target discrete value
    ...
    def _check_termination(self, obs):
        ...
        continuous_distance = np.abs(obs['continuous_obs'][0] - self.target_continuous_value)
        target_achieved = (continuous_distance < 0.1 and 
                          obs['discrete_obs'] == self.target_discrete_value)
...
class FlattenedDictEnv(gym.Wrapper):
    ...
    def _flatten_observation(self, obs_dict):
        ...
        continuous = obs_dict['continuous_obs'].astype(np.float32)
        ...
        flattened = np.concatenate([continuous, discrete_onehot, temporal])
